...
function displayRecentActivity(activities) {
    ...
    container.innerHTML = activities.map(activity => `
        <div class="activity-item">
            <p><strong>${escapeHtml(activity.type)}:</strong> ${escapeHtml(activity.description)}</p>
...
function displayUsers(users) {
    ...
    container.innerHTML = users.map(usr => `
        <div class="user-card">
            <h3>${escapeHtml(usr.fullName)}</h3>
            <p><strong>Email:</strong> ${escapeHtml(usr.email)}</p>
            <p><strong>Phone:</strong> ${escapeHtml(usr.phone) || 'N/A'}</p>
            <p><strong>Role:</strong> ${usr.role.toUpperCase()}</p>
            ${usr.role === 'doctor' ? `<p><strong>Specialization:</strong> ${escapeHtml(usr.specialization)}</p>` : ''}
...
function displayAllAppointments(appointments) {
    ...
    container.innerHTML = appointments.map(apt => `
        <div class="appointment-card">
            <h3>Patient: ${escapeHtml(apt.patient.fullName)}</h3>
            <p><strong>Doctor:</strong> ${escapeHtml(apt.doctor.fullName)} (${escapeHtml(apt.doctor.specialization)})</p>
